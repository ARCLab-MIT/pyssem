...
# ---------- user inputs ----------
PATH = '/Users/indigobrownhall/Code/pyssem/pyssem/utils/launch/data/ref_scen_SEP2C.csv'
...
# convert all species classe of Su to S
T.loc[T['species_class'] == 'Su', 'species_class'] = 'S'
...
for sp, ks in species_k_map.items():
    ...
    # Apply species-specific year filtering
    if sp == "N":
        # For N: year_start <= 2025
        df_sp = df_sp[df_sp["year_start"] <= 2025].copy()
    elif sp in ("S", "B"):
        # For S and B: year_start >= 2025 and year_start <= 2040
        df_sp = df_sp[(df_sp["year_start"] >= 2025) & (df_sp["year_start"] <= 2040)].copy()
    # Other species (if any) use all available data

    if df_sp.empty:
        print(f"[WARN] No rows for species {sp} after year filtering; skipping.")
        continue
...
try:
    ...
    # Apply year filter for N: year_start <= 2025
    df_N = df_N[df_N["year_start"] <= 2025].copy()
    if df_N.empty:
        print("[N elbow] No debris rows found after year filtering; skipping debris elbow plot.")
    else:
...
try:
    ...
    eps = 1e-6
    # Apply species-specific year filtering for eccentricity analysis
    df_N_ecc = assigned_df[assigned_df["species_class"] == "N"].copy()
    df_N_ecc = df_N_ecc[df_N_ecc["year_start"] <= 2025].copy()
    
    df_B_ecc = assigned_df[assigned_df["species_class"] == "B"].copy()
    df_B_ecc = df_B_ecc[(df_B_ecc["year_start"] >= 2025) & (df_B_ecc["year_start"] <= 2040)].copy()
    
    ecc_species = {
        "N": df_N_ecc["ecc"].to_numpy(float) if not df_N_ecc.empty else np.array([]),
        "B": df_B_ecc["ecc"].to_numpy(float) if not df_B_ecc.empty else np.array([])
    }
