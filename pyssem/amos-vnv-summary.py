...
def process_scenario(sim_dir: Path, mc_dir: Path,
    ...
    mc_tot_path   = mc_dir / "pop_time.csv"
    mc_alt_path   = mc_dir / "pop_time_alt.csv"
    ...
    if not mc_tot_path.exists() or not mc_alt_path.exists():
        raise FileNotFoundError("MC directory must contain pop_time.csv and pop_time_alt.csv")
...
def plot_scenario_scores(scores: list[tuple[str,float]], out_dir: Path):
    ...
    out_pdf = out_dir / "scenario_scores.pdf"
...
def plot_group_heatmap(all_group_df: pd.DataFrame, out_dir: Path):
    ...
    out_pdf = out_dir / "group_scores_heatmap.pdf"
    plt.savefig(out_png, dpi=180)
    plt.savefig(out_pdf)
    plt.close()
    print(f"📊 Wrote {out_png} and {out_pdf}")
...
def plot_score_cpu_pareto(scores_df: pd.DataFrame, out_dir: Path):
    ...
    out_pdf = out_dir / "score_vs_cpu_pareto.pdf"
    plt.savefig(out_png, dpi=180)
    plt.savefig(out_pdf)
    plt.close()
    ...
    print(f"📉 Wrote {out_png} and {out_pdf}; saved Pareto set to {pareto_csv}")
...
def mc_species_weights(mc_dir: Path) -> dict:
    """
    Compute species weights (MC total mass share across all years) from MC pop_time.csv.
    """
    mc_tot_path = mc_dir / "pop_time.csv"
    df_mc_tot = read_pop_time_csv(mc_tot_path)
...
def plot_pareto_component_stacks(scores_df: pd.DataFrame, all_group_df: pd.DataFrame, species_weights: dict, out_dir: Path):
    ...
    out_pdf = out_dir / "pareto_component_stacks.pdf"
    plt.savefig(out_png, dpi=180); plt.savefig(out_pdf); plt.close()
    print(f"🧱 Wrote {out_png} and {out_pdf}")
...
def plot_score_cpu_colored(df: pd.DataFrame, out_dir: Path):
    ...
    # Map model types to integer codes for colors (matplotlib will choose palette)
    mtypes = sorted(d["model_type"].astype(str).unique())
    mt_map = {m:i for i,m in enumerate(mtypes)}
    colors = [mt_map[m] for m in d["model_type"].astype(str)]
    sizes = 40 + 4 * d["n_shells"].astype(float).fillna(d["n_shells"].median() if "n_shells" in d else 20)

    plt.figure(figsize=(9,6))
    sc = plt.scatter(d["cpu_seconds"], d["Score_scenario"], c=colors, s=sizes, alpha=0.75)
    # Build a simple legend for model types
    for m in mtypes:
        mask = d["model_type"].astype(str) == m
        if mask.any():
            plt.scatter([], [], label=m, s=80, alpha=0.9)
    plt.xlabel("CPU time (seconds)")
    plt.ylabel("Composite scenario score (lower is better)")
    ...
    pdf = out_dir / "score_vs_cpu_colored.pdf"
...
def run_factor_importance(df_in: pd.DataFrame, out_dir: Path, all_group_df: pd.DataFrame | None = None):
    ...
    try:
        ...
        fig.savefig(out_dir / "importance_compact_heatmap.pdf")
        plt.close(fig)
        ...
        if all_group_df is not None and not all_group_df.empty:
            ...
            if col_map:
                ...
                fig2.savefig(out_dir / "importance_compact_species_heatmap.pdf")
                plt.close(fig2)
    except Exception as e:
...
def plot_anova_share_heatmap(scores_df: pd.DataFrame, all_group_df: pd.DataFrame, out_dir: Path):
    ...
    fig.savefig(out_dir / "importance_compact_anova_shares.pdf")
    plt.close(fig)
...
def main():
    ...
    Configure default paths here or via environment variables:
      ...
      - SSEM_MC:   folder containing MC truth pop_time.csv and pop_time_alt.csv
    ...
    ROOT_DIR_DEFAULT = "./grid_search_new"              # change if needed
    MC_DIR_DEFAULT   = "/Users/indigobrownhall/Code/MOCAT-VnV/results/"                 # change if needed
